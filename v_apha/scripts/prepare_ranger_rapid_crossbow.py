#!/usr/bin/env python3
"""Promote Ranger's rapid crossbow and validate its inherited-base rotation."""
...
NAME = "ranger__active_primary__rapid_crossbow_v1"
...
def bbox(image: Image.Image) -> tuple[int, int, int, int]:
    ...
    if not result:
        raise RuntimeError("empty Ranger rapid-crossbow cutout")
    return result
...
def main() -> None:
    ...
    # ImageGen was constrained to center the mechanical axle on the source canvas.
    # Preserve that authored socket center instead of using center of visible mass.
    source_pivot = [source.width // 2, source.height // 2]
    ...
    spill = sum(1 for r, g, b, a in cutout.getdata()
                if a > 18 and r > 160 and b > 140 and g < 115 and min(r-g, b-g) > 65)
    ...
    manifest = {
        ...
        "attachments": {
            ...
            "magazine_socket": {"positionPx": [round(cutout.width * .66), pivot[1]], "positionNormalized": [0.66, round(pivot[1] / cutout.height, 6)], "inheritRotation": True},
            "optics_socket": {"positionPx": [round(cutout.width * .67), max(0, pivot[1] - round(cutout.height * .12))], "inheritRotation": True},
    ...
    for index, angle in enumerate((0, 90, 180, 270)):
        ...
        ImageDraw.Draw(grid).text((col*tile+12, y+14), f"RANGER ROTATION {angle:03d} DEG", font=font, fill="#f3ead3")
