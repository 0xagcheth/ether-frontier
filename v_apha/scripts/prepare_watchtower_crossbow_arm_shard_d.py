#!/usr/bin/env python3
"""Promote Watchtower crossbow-arm debris and validate it against the weapon."""
...
SOURCE = CANDIDATES / "watchtower__debris__crossbow_arm_shard_d_v1_chroma.png"
...
NAME = "watchtower__debris__crossbow_arm_shard_d_v1"
SCALE_TO_WEAPON_LENGTH = 0.36
...
def main() -> None:
    ...
    manifest = {
        "schemaVersion": 1,
        "module": "watchtower__debris__crossbow_arm_shard_d",
        "object": "watchtower",
        ...
        "destroyPhysics": {
            "massClass": "light_wood",
            "inheritOwnerRotation": True,
            "radialVelocityPxPerSec": [155, 255],
            "angularVelocityDegPerSec": [-420, 420],
            "linearDrag": 0.885,
            "lifetimeMs": [500, 900],
        },
        "drawOrder": 64,
    ...
    game_shard = crop.resize(tuple(game_size), Image.Resampling.LANCZOS)
    tile, header = 860, 52
    grid = Image.new("RGB", (tile * 2, tile + header), "#292a2b")
    font = ImageFont.load_default()
    for index, (image, source_image, source_pivot) in enumerate((
        (game_weapon, weapon, weapon_meta["pivotPx"]),
        (game_shard, crop, pivot),
    )):
        ...
        draw_panel.line((px - 11, py, px + 11, py), fill="#e34b46", width=3)
        draw_panel.line((px, py - 11, px, py + 11), fill="#e34b46", width=3)
    ...
    draw.text((tile + 14, 18), f"ARM SHARD D  {game_size[0]} x {game_size[1]} px", font=font, fill="#f3ead3")
    review_path = REVIEW / "watchtower__crossbow_arm_shard_d_v1_review.png"
    grid.save(review_path, optimize=True)

    print(json.dumps({
        "manifest": str(manifest_path.relative_to(ROOT)), "alpha": str(alpha_path.relative_to(ROOT)),
        "tightSize": list(crop.size), "centerOfMassPivotPx": pivot, "weaponGameSize": weapon_size,
        "composedSize": game_size, "magentaSpillVisible": spill,
